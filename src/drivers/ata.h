// thanks to:
//  https://github.com/szhou42/osdev/blob/master/src/include/ata.h
//  https://github.com/szhou42/osdev/blob/master/src/kernel/drivers/ata.c
...
void ata_device_init(ata_dev_t * dev, int primary) {
    ...
    dev->prdt = (void*)malloc(sizeof(prdt_t)); // CHECKME: works the same as kmalloc_a(sizeof(prdt_t)) from https://github.com/szhou42/osdev/blob/master/src/kernel/mem/kheap.c#L103 (also for line 334!!!)
    memset(dev->prdt, 0, sizeof(prdt_t));
    dev->prdt_phys = virtual2phys(/*kpage_dir, */dev->prdt);
    dev->mem_buffer = (void*)malloc(4096);
...
void ata_device_detect(ata_dev_t * dev, int primary) {
    ...
    if(!inportb(dev->status)) {
        com_print(COM1, "ata_detect_device: device does not exist\n");
        return;
    }
    ...
    if(lba_lo != 0 || lba_hi != 0) {
        com_print(COM1, "ata_detect_device: not ata device\n");
        return;
    }
    // Polling
    uint8_t drq = 0, err = 0;
    ...
    if(err) {
        com_print(COM1, "ata_detect_device: err when polling\n");
        return;
    }
